...
class database extends base {
	...
	private function query ($query, $save = false) {
	...
	private function select_columns ($tablename) {
		$xml = simplexml_load_string (file_get_contents (common::root_path ("Data/Library/db.tables.xml")));
		return $this->query ($this->parameterize ((string) $xml->select_columns [0], array (
			"schema"	=> $this->schema,
			"tablename"	=> $tablename
		)));
	}// select_columns;
	...
	private function table_columns ($tablename, $parameters) {
		...
		while ($column = $columns->fetch_assoc ()) {
			...
			$result [$column_name] = in_array ($column ["DATA_TYPE"], array ("int", "tinyint")) ? $column_value : "'{$column_value}'";
		}// foreach;
		return $result;
	}// table_columns;
	...
	private function update ($tablename, $parameters, $id) {
		$query = "update {$tablename} set ({$this->update_parameters ($parameters)}) where id = {$id};";
	...
	public function save ($tablename, $parameters, $id = null) {
		...
		return update ($tablename, $parameters, $id);
	}// save;
